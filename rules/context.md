...
- **대형 문서(수천 줄)는 통째로 읽지 않는다** — `grep -n "^## "`로 목차를 보고 필요한 절만 offset/limit로 읽는다.
- 판정 근거로 삼는 함수는 **끝까지 읽는다** — 부분 읽기만으로 "정상"이라 판단하지 않는다.
...
- 각 호출에 단일 목표 + 산출물 명시.
- **서브에이전트 보고값은 그대로 쓰지 않는다** — 결론이 걸린 값(경로·라인·수치·"없다")은 원본에서 직접 재확인한 뒤 반영한다. 요약은 탐색의 지도이지 근거가 아니다.
