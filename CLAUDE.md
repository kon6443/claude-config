...
산출물 골격은 스킬이다: `/plan`, `/bugfix`, `/session-handoff`, `/task-folder`. 필요하면 직접 호출하고, 상황이 맞으면 알아서 불러온다.

> **프로젝트가 우선한다.** 여기 내용과 프로젝트의 `CLAUDE.md`·`.claude/rules/`가 충돌하면 프로젝트 쪽을 따른다.
> 로드 순서가 우선순위를 정하지는 않는다 — 하네스는 사용자·프로젝트 지시를 모두 컨텍스트에 넣을 뿐, 충돌 시 어느 쪽이 따라질지는 보장하지 않는다(공식 memory 문서).
> 그래서 이 문장 자체가 규칙이다: 충돌을 발견하면 프로젝트 쪽을 따르고, 충돌 사실을 응답에 1줄 밝힌다.
...
2. **검증 증거 존재** — 테스트/lint/typecheck/빌드 통과, 또는 수행하지 않은 사유와 사용자가 직접 검증할 수 있는 명령 목록 제공.
   - 레포에 **선재 에러**(lint·typecheck)가 있으면 "전체 통과"가 아니라 **변경 파일의 신규 에러 0**을 base 버전과 개수 비교로 판정한다 (예: `git show <base>:<f>`를 stdin으로 넘겨 같은 도구로 센다).
