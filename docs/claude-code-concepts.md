...
**위치와 범위** (넓은 것부터 로드된다. 전부 컨텍스트에 함께 들어갈 뿐 **충돌 시 우선순위는 보장되지 않는다** — 공식 문서: "Claude may follow either one". settings.json 처럼 좁은 쪽이 이기는 구조가 아니다):
...
**이 저장소**: 7개 파일. 5개는 조건 없음(엔지니어링·에러 복구·git 위생·워크플로·컨텍스트 관리), 2개는 `paths:` 조건부(셸 이식성, 3층 태스크 폴더).
...
**이 저장소**: 7개. `/plan`·`/bugfix`·`/session-handoff`·`/task-folder`·`/review`는 자동 호출 허용(상황에 맞춰 불리는 게 이득), `/pr-desc`·`/tasks-dashboard`는 자동 호출 차단(사람이 의도적으로 돌리는 절차).
