...
**프로젝트가 우선한다.** 단 이는 하네스 동작이 아니라 CLAUDE.md 에 적은 **명시적 지시**다 — 공식 memory 문서는 사용자·프로젝트 규칙이 충돌하면 "Claude may follow either one"이라고 한다. 로드 순서에 기대지 말고 두 규칙을 일관되게 유지하되, 충돌 시에는 프로젝트를 따르라고 지시해 둔다.
